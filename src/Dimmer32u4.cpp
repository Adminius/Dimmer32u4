...
Dimmer32u4::Dimmer32u4(uint8_t pin)
{
    _pin = pin;
    _invert = false;
}

Dimmer32u4::Dimmer32u4(uint8_t pin, bool invert)
{
    _pin = pin;
    _invert = invert;
}
...
void Dimmer32u4::setValue(uint16_t value)
{
    uint16_t normalizedValue = value;
    if (value > _maxValue){
...
uint16_t Dimmer32u4::getMaxValue(){
    return _maxValue;
}

uint8_t Dimmer32u4::getResolution(){
    return _resolution;
}
